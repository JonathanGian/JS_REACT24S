...
function task2() {
  try{
    ...
    if (error instanceof ReferenceError) {
      console.log("Referance error:", error.message);
    } else {
...
function task3() {
  try {
    throw new Error("An issue has been detected.");
...
function task4() {
  try {
    ...
    try {
      ...
      throw new Error("Issue in inner nest");
      
    } catch (innerError) {
      console.error("Caught an inner error:",innerError.message);
    }
    throw new Error("outside error message")
  } catch (outError) {
    console.error("Caught an outer error",outError.message);
  }finally{
...
console.log(task5([1,2,3],1))// this will show the "completed" before 2
...
task6({firstName:"Jonathan"},"firstName")
